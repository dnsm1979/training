...
def callback(call):
    ...
    bot.send_message(
        call.message.chat.id, f"Ваш результат: {ls_score[0]}+{ls_score[1]}"
    )
    time.sleep(3)
    bot.send_message(call.message.chat.id, "Теперь кидаю я!")
    time.sleep(3)
    bot.send_message(
        call.message.chat.id, f"Мой результат: {ls_score[2]}+{ls_score[3]}"
    )

    if ls_score[0] + ls_score[1] == ls_score[2] + ls_score[3]:
        bot.send_message(
            call.message.chat.id,
            f"{ls_score[0]}+{ls_score[1]} против {ls_score[2]}+{ls_score[3]}: Розыгрыш!",
        )
    elif ls_score[0] + ls_score[1] < ls_score[2] + ls_score[3]:
        bot.send_message(
            call.message.chat.id,
            f"{ls_score[0]}+{ls_score[1]} против {ls_score[2]}+{ls_score[3]}: Вы проиграли!",
        )
    else:
        bot.send_message(
            call.message.chat.id,
            f"{ls_score[0]}+{ls_score[1]} против {ls_score[2]}+{ls_score[3]}: Вы выйграли!",
        )
    ls_score.clear()
    markup = types.InlineKeyboardMarkup()
