...
def add_worker() -> None:
    try:
        contact = input('Введите данные работника через пробел\n'
                        ' (тел. ФИО email должность кабинет Skype) -> ')
        key_ls = phone, l_name, f_name, surname, email, job, cabinet, skype = contact.split(' ')
        ...
        print('Неверный формат ввода!\n')
        main()
    vel_ls = ['Фамилия', 'Имя', 'Отчество', 'рабочий EMAIL', 'должность', 'номер кабинета', 'Skype']
...
def re_worker() -> None:
    dict = {'1':'Фамилию', '2':'Имя', '3':'Отчество', '4':'рабочий EMAIL', '5':'должность', '6':'номер кабинета', '7':'Skype'}
    phone = input('Введите телефон работника -> ')
    print('\n')
    if phone in db_workers.keys():
        choice = input('Выберете такую информацию о работнике вы хотите изменить:\n'
...
def main() -> None:
    while True:
        choice = input('1-Добавить работника\n'
